...
- **`Cron.php`**: auth via `?token=` yang dicocokkan ke `cron_token` di secrets, atau bebas token saat `is_cli()`. Isinya sekarang hanya `auto_upload_*` (dipanggil `scripts/*.py`), `check_resi_status`, `fix_resi_detail`, `tutup_video_menggantung`, `finalisasi_video` (task scheduler, tiap menit), dan `sinkron_foto_sku` (task harian 23.00: salin foto produk ke `C:oto-produk\` + isi `tblsku.foto_lokal`; browser pakai URL asli dulu, lokal cadangan lewat `assets/js/foto_sku.js`; Apache `Alias /foto-produk/`; helper `foto_sku_img()`/`foto_sku_lokal_url()`).
