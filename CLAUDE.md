...
Alur bisnis inti: **Receipt → Picking → Packing → Handover → Shipped**, dengan jalur paralel **Retur → Buka Retur → QC/Repair/Reject → Restock/Display → Finance/Accounting**. Peta lengkap area → controller → model → tabel ada di `HANDOFF.md` §2; detail per-controller di `docs/ANALISIS_PROGRAM.md`, skema tabel di `docs/DATABASE_STRUCTURE.md`, diagram alur di `docs/WORKFLOW_DIAGRAM.md`, alur lost scan lintas meja (HO → packer → tim picker) di `docs/LOST_SCAN.md`, alur menu packer Salah Ambil Special (batch resi 1 SKU/1 qty salah ambil → `tblmasalahpicker`) di `docs/SALAH_AMBIL_SPECIAL.md`, alur paket cancel (scan ditolak karena `CANCELED` → `tblcancel_paket_tolak`/`tblcancel_paket` via `Cancel_paket_fcd::catat_tolak()` di 7 titik penolakan → tim retur → restock; tahap 1 live, 2–3 rancangan) di `docs/PAKET_CANCEL.md`, aturan "wajib keluar hari ini" (12.00 semua MP, TikTok 15.00 bila batas kirim ≤ besok, cancel keluar dari angka, SHIPPED tanpa scan HO = sudah keluar) + hitungan OT/perbantuan packer untuk menu Tim Monitoring **Pemenuhan Kirim Harian** di `docs/PEMENUHAN_KIRIM_HARIAN.md`.
...
- **`Cron.php`**: auth via `?token=` yang dicocokkan ke `cron_token` di secrets, atau bebas token saat `is_cli()`. Isinya sekarang hanya `auto_upload_*` (dipanggil `scripts/*.py`), `check_resi_status`, `fix_resi_detail`, `tutup_video_menggantung`, `finalisasi_video` (task scheduler, tiap menit), dan `sinkron_foto_sku` (task harian 23.00: salin foto produk ke `C:oto-produk\` + isi `tblsku.foto_lokal`; browser pakai URL asli dulu, lokal cadangan lewat `assets/js/foto_sku.js`; Apache `Alias /foto-produk/`; helper `foto_sku_img()`/`foto_sku_lokal_url()`), dan `arsip_harian` (task 01.00 "IRESIS - Arsip harian": salin prod → `iresis_arsip` sebagai superset via `REPLACE INTO … SELECT` per batch dengan watermark di `iresis_arsip._arsip_status`; logika di `models/Arsip_fcd.php`, setelan `config/arsip.php`; job ini **tidak pernah** DELETE/UPDATE di prod). **Mode Arsip**: menu `arsip` (akses = roleaccess) memasang session `mode_arsip`; `MY_Controller::terapkan_mode_arsip()` di **akhir** constructor meng-`db_select` ke arsip + `SET SESSION TRANSACTION READ ONLY` — kalau menambah kode di constructor yang harus menyentuh prod, letakkan **sebelum** pemanggilan itu. Helper autoload `pastikan_resi_live($noresi)` (→ `Arsip_fcd::tarik_balik`, peta tabel `keluarga_resi` di config) dipanggil di pintu masuk retur/CS/detail resi/cancel order — kalau menambah alur baru yang mencari resi lama by `noresi`, panggil itu dulu. Backup arsip: task 02.30 via `scripts/backup_arsip_senyap.vbs`. **Purna** (`Cron::arsip_purna`, aktif di putaran malam sejak 21 Sep 2026 lewat `purna_aktif`) adalah satu-satunya kode yang menghapus di prod — per batch REPLACE → verifikasi PK di arsip → DELETE, dengan gerbang backup/sinkron ≤ 36 jam. `Cron::optimasi_tabel` (task 22.30 tgl 1) mengembalikan ruang disk. Seluruh alur di `docs/ARSIP_DATA.md`.
