...
<h2 style="font-family: Calibri; color: #1b2631;">&#128203; LAPORAN PRODUKSI HANDOVER (HO)</h2>
...
<table style="border:none; margin-bottom: 10px;" class="no-border">
    ...
    <tr>
        ...
        <td style="border:none;">:</td>
        <td style="border:none; font-weight:bold; color:#1b2631;"><?= $grand_total ?></td>
    </tr>
    </tbody>
</table>
...
foreach ($list_inti as $pegawai => $items) {
    $sub_total = 0;
    $rows_inti[] = ['type' => 'name', 'no' => $i_inti++, 'nama' => $pegawai];
    foreach ($items as $item) {
        $sub_total += $item['total'];
        $rows_inti[] = ['type' => 'item', 'tanggal' => empty($item['tanggal']) ? '' : date('d/m/Y H:i', strtotime($item['tanggal'])), 'total' => $item['total'], 'status_performa' => $item['status_performa'] ?? ''];
    }
    $rows_inti[] = ['type' => 'subtotal', 'sub_total' => $sub_total];
}

$i_perb = 1;
foreach ($list_perbantuan as $pegawai => $items) {
    $sub_total = 0;
    $rows_perb[] = ['type' => 'name', 'no' => $i_perb++, 'nama' => $pegawai];
    foreach ($items as $item) {
        $sub_total += $item['total'];
        $rows_perb[] = ['type' => 'item', 'tanggal' => empty($item['tanggal']) ? '' : date('d/m/Y H:i', strtotime($item['tanggal'])), 'total' => $item['total'], 'status_performa' => $item['status_performa'] ?? ''];
    }
    $rows_perb[] = ['type' => 'subtotal', 'sub_total' => $sub_total];
}
...
<table>
    <thead>
        <tr>
            <th colspan="5" class="header-inti">&#11088; HO INTI</th>
            <td class="spacer-col" rowspan="<?= $max_rows + 2 ?>"></td>
            <th colspan="5" class="header-perbantuan">&#128100; PERBANTUAN / LAIN-LAIN</th>
        </tr>
        <tr>
        ...
        <tr>
            <?php if (isset($rows_inti[$r])): ?>
                ...
                <?php if ($ri['type'] === 'name'): ?>
                    ...
                    <td class="row-name-inti" colspan="3"><?= htmlspecialchars($ri['nama']) ?></td>
                ...
                <?php elseif ($ri['type'] === 'subtotal'): ?>
                    ...
                    <td class="subtotal-inti"></td>
                    <td class="subtotal-inti" align="right">&#128205; Total</td>
            ...
            <?php if (isset($rows_perb[$r])): ?>
                ...
                <?php if ($rp['type'] === 'name'): ?>
                    ...
                    <td class="row-name-perbantuan" colspan="3"><?= htmlspecialchars($rp['nama']) ?></td>
                ...
                <?php elseif ($rp['type'] === 'subtotal'): ?>
                    ...
                    <td class="subtotal-perbantuan"></td>
                    <td class="subtotal-perbantuan" align="right">&#128205; Total</td>
    ...
    <tr>
        <td class="grand-total-row" colspan="5" align="right">TOTAL KESELURUHAN (INTI): <?= number_format($total_paket_inti) ?> paket</td>
        <td class="spacer-col"></td>
        <td class="grand-total-row" colspan="5" align="right">TOTAL KESELURUHAN (PERBANTUAN): <?= number_format($total_paket_perbantuan) ?> paket</td>
    </tr>
    </tbody>
</table>
...
<table style="width: 500px;">
    <thead>
        <tr>
            <th class="summary-header" colspan="4">&#128202; RANGKUMAN PRODUKSI HANDOVER (HO)</th>
        </tr>
        <tr>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="summary-inti-val" style="color:#1b2631;"><?= $total_masuk_inti ?></td>
