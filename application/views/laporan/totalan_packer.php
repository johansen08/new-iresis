<div class="row">
  <div class="col-md-12">
    <div class="panel panel-default">
      <div class="panel-heading">
        <h3 class="panel-title">
          Totalan Packer (Realtime)
          <span class="pull-right text-muted" id="last-refresh-packer"></span>
        </h3>
      </div>
      <div class="panel-body">
        <div class="row margin-bottom-15">
          <div class="col-md-4">
            <div class="alert alert-success text-center" style="margin-bottom:0">
              <h2 id="grand-total-packer" style="margin:0">0</h2>
              <small>Total Resi Hari Ini</small>
            </div>
          </div>
          <div class="col-md-8 text-right" style="padding-top:15px">
            <span class="text-muted"><i class="fa fa-refresh fa-spin"></i> Auto-refresh setiap 30 detik</span>
          </div>
        </div>

        <table class="table table-striped table-bordered" id="tbl-packer">
          <thead>
            <tr>
              <th width="40">No</th>
              <th>Nama Packer</th>
              <th width="100" class="text-center">Total Resi</th>
              <th width="100" class="text-center">Satuan</th>
              <th width="100" class="text-center">Campuran</th>
              <th width="100" class="text-center">Total SKU</th>
              <th width="120" class="text-center">Total Kesalahan</th>
              <th width="100" class="text-center">Total Point</th>
            </tr>
          </thead>
          <tbody>
            <?php if (!empty($rows)): $i = 1; foreach ($rows as $row): ?>
            <tr>
              <td><?= $i++ ?></td>
              <td><?= $row->nama_packer ?></td>
              <td class="text-center"><strong><?= $row->total_resi ?></strong></td>
              <td class="text-center"><?= $row->satuan ?></td>
              <td class="text-center"><?= $row->campuran ?></td>
              <td class="text-center"><?= $row->total_sku_qty ?></td>
              <td class="text-center"><?= $row->total_kesalahan ?></td>
              <td class="text-center"><span class="badge badge-success"><?= $row->total_point ?></span></td>
            </tr>
            <?php endforeach; endif; ?>
          </tbody>
        </table>
      </div>
    </div>
  </div>
</div>

<script>
(function() {
  function refreshData() {
    $.post('laporan/get-data-totalan-packer', {}, function(res) {
      var data = (typeof res === 'string') ? JSON.parse(res) : res;
      var tbody = $('#tbl-packer tbody');
      tbody.empty();

      if (data.data && data.data.length) {
        data.data.forEach(function(row) {
          tbody.append(
            '<tr>' +
              '<td>' + row[0] + '</td>' +
              '<td>' + row[1] + '</td>' +
              '<td class="text-center"><strong>' + row[2] + '</strong></td>' +
              '<td class="text-center">' + row[3] + '</td>' +
              '<td class="text-center">' + row[4] + '</td>' +
              '<td class="text-center">' + row[5] + '</td>' +
              '<td class="text-center">' + row[6] + '</td>' +
              '<td class="text-center"><span class="badge badge-success">' + row[7] + '</span></td>' +
            '</tr>'
          );
        });
      } else {
        tbody.append('<tr><td colspan="5" class="text-center text-muted">Belum ada data hari ini</td></tr>');
      }

      $('#grand-total-packer').text(data.grandTotal || 0);
      $('#last-refresh-packer').text('Update: ' + moment().format('HH:mm:ss'));
    });
  }

  var _packerInterval = setInterval(refreshData, 30000);
  refreshData();

  $(document).on('pjax:end', function() { clearInterval(_packerInterval); });
})();
</script>
